...
test("calculates a projection from form inputs", async ({ page }) => {
  ...
  await page.getByLabel("국민연금 정상 월수령액").fill("1,550,000");
  await page.getByLabel("국민연금 선택 개시 연령").fill("65");
  await page.getByLabel("주택연금 월수령액").fill("850,000");
  await page.getByLabel("주택연금 개시 연령").fill("70");
  await page.getByLabel("IRP 현재 잔액").fill("70,000,000");
  await page.getByLabel("IRP 월 납입 계획").fill("600,000");
  await page.getByLabel("IRP 연 예상수익률(%)").fill("4.2");
  await page.getByLabel("주식/투자 현재 잔액").fill("110,000,000");
